...
- **Onboarding gates on the permission that is no longer required.** `BreatheNavGraph` picks its start destination with `if (hasUsage) Routes.HOME else Routes.ONBOARDING` (`ui/nav/NavGraph.kt:34`). Usage access became optional in `90f7e30`, so a user who granted accessibility and overlay but declined usage access lands on onboarding at every cold launch. `OnboardingScreen` does auto-navigate away once the two real permissions are present, so nobody is trapped — but `startDestination` is only read at first composition, which makes it a visible flash that tells a fully-configured user their setup is incomplete. The fix is to key it on the same pair `HomeViewModel.isMonitoringActive` already uses. Found while revising the planning documents, not by the device pass.
- **`HomeViewModel` recomputes a 7-day usage aggregate inside a Flow collector.** `loadAppsWithStats()` (`ui/home/HomeViewModel.kt:63-77`) calls `queryAndAggregateUsageStats` *inside* `repo.getBlockedApps().collect { }`, so the whole aggregate re-runs on every emission — every app added, removed, or re-timed. It also pulls `UsageStatsManager` out of an injected `Context` rather than from `SystemServiceModule`, which is a large part of why the class has no tests. This is the same shape as the bounded-scan problem already fixed once in `SessionTimeHelper`.
