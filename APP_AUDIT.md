...
- `README.md` roadmap is accurate and its "How it works" section was updated with the rewrite, but the **Permissions** section (`README.md:73-79`) was not. It still states the app "requires one special permission", names usage access as that permission, and does not mention accessibility access or display-over-other-apps at all. This is the inverse of the truth: usage access is the one permission the app no longer needs. Worth fixing ahead of the `.planning/` documents, since it is the file a new contributor reads first and the only one here that is user-facing.
