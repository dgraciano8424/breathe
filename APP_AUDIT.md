...
1. **The interception path has never been observed working.** **Addressed in code twice, still unverified on hardware.**
   ...
   **Detection — the accessibility service** (`90f7e30`). `BreatheAccessibilityService` receives `typeWindowStateChanged` events instead of polling `UsageStatsManager` every 500ms. The event arrives as the app comes to the front, so the pause can appear before the blocked app draws rather than up to half a poll interval later. The blocked set is mirrored into memory on `onServiceConnected`, so the event path never touches the database.
   ...
   - New risk introduced with it: accessibility services are user-disabled from a settings screen the app does not control, and some OEM battery managers switch them off silently. The failure mode "monitoring quietly stopped" is now more likely than it was with a foreground service holding a visible notification. **That notification was also the user's only ambient signal that the app was running, and it is gone.**
   - Mitigation as of 2026-08-14: the home screen shows a "monitoring is off" card when accessibility or overlay permission is missing, refreshed on resume. Note that until that date this document claimed the same mitigation already existed — `90f7e30` exposed `isMonitoringActive` on `HomeViewModel` but never rendered it, so the state was computed, never shown, and stale besides. See corrections.
...
6. ~~**The unit-test suite is out of sync with production APIs.**~~ **Resolved**, then **partly undone** by `90f7e30`. The suite no longer tests anything that does not exist — `ForegroundAppDetectorTest` and `QuoteRepositoryTest` went with the code they covered, and `PauseViewModelTest` matches the current constructor and reason semantics. It is in sync.
   ...
   It is also smaller: 48 tests across 5 classes, down from 67 across 6 — and 43 immediately after the rewrite, before `HomeViewModelTest` was added. The drop is not itself a defect, since deleting tests for deleted code is correct, but 250 of those lines covered detection, and detection still exists. It was reimplemented, not removed. See "Test coverage" under corrections.
...
- **No ambient signal that monitoring is alive.** Partly addressed, not closed. The home screen now shows a "monitoring is off" card (below), but a user whose OEM battery manager silently disabled accessibility will not be looking at the home screen — that is the one place the app is not, by definition, when monitoring should be happening. The foreground-service notification used to cover exactly that gap. Worth deciding deliberately whether a silent stop is acceptable, rather than discovering the answer from a review.

- Nothing outstanding beyond test coverage for detection and the monitoring-signal decision above. Both items found during the 2026-08-14 documentation pass are fixed — see below.

Fixed 2026-08-14, after the documentation pass surfaced them:

- ~~**Onboarding gates on the permission that is no longer required.**~~ **Resolved.** `BreatheNavGraph` chose its start destination with `if (hasUsage)`, but usage access became optional in `90f7e30`, so anyone who declined it landed on onboarding at every cold launch despite a working setup. The predicate now lives in `OnboardingViewModel.isSetupComplete()` and reads accessibility plus overlay — the two permissions interception actually needs, and the same pair `HomeViewModel.isMonitoringActive` uses. It is wrapped in `remember`, since `NavHost` reads `startDestination` once and recomputing it later cannot re-route anything.
- ~~**`HomeViewModel` recomputes a 7-day usage aggregate inside a Flow collector.**~~ **Resolved.** `queryAndAggregateUsageStats` ran *inside* `getBlockedApps().collect { }`, so a full 7-day scan re-ran on every emission — every app added, removed, or re-timed. Usage totals now live in their own `StateFlow`, `combine`d with the blocked-apps Flow and refreshed on their own schedule. `UsageStatsManager` is injected from `SystemServiceModule` instead of being pulled from an `@ApplicationContext`, and a missing usage grant now degrades to zeros via `runCatching` rather than depending on the platform returning an empty map.
  - That injection seam is what made the class testable. `HomeViewModelTest` is new — 5 tests, including a regression test asserting the aggregate runs exactly once across multiple blocked-list changes, which is the specific defect that was fixed.
- ~~**Monitoring state was computed but never shown, and never refreshed.**~~ **Resolved.** `90f7e30` added `HomeViewModel.isMonitoringActive` and this audit recorded it as the mitigation for a silently-disabled service. It was not one: **no composable ever collected it**, so the state was calculated and discarded. It was also only ever computed in `init`, and since revoking a permission happens in Settings — which does not take `HomeScreen` out of composition — even a rendered version would have gone stale immediately.
  - `HomeScreen` now collects it and shows a "monitoring is off" card above everything else, because every other figure on that screen is a count of past pauses: with monitoring off the app looks like it is working and simply has nothing to report yet, which is the worst available failure mode for this product.
  - Refresh moved to a `Lifecycle.Event.ON_RESUME` observer, matching the pattern `OnboardingScreen` already used for the same reason.
  - The card routes back to onboarding rather than deep-linking into Settings, so the accessibility prominent-disclosure and consent step cannot be bypassed — that flow is a Play requirement and should keep exactly one owner.
  - **This one is a caution about the audit itself.** The claim "surfaces it on the home screen" was carried in this document, in `RELEASE.md`, in `CONCERNS.md` and in `ARCHITECTURE.md` on the strength of a commit message, and none of it was true. It was found by grepping for consumers of the flow while fixing something else.
...
- ~~All six `.planning/codebase/` documents~~ **Fixed 2026-08-14**, re-derived against the tree rather than patched. They had been dated 2026-06-15 throughout — a month before the original audit — so they were two architectures behind, not one, and described a polling foreground service, a boot receiver, and a quote/network layer that no longer exist.

  Worth recording what that drift had turned into, because it is an argument for checking these documents against source rather than trusting them:
  - `TESTING.md` stated "there are no test files in this project" and offered speculative examples mocking `ZenQuotesApi` and `ForegroundAppDetector`. There were 43 tests, and neither class existed.
  - `CONVENTIONS.md` stated that no logging framework was used and that nothing carried KDoc. Both are now standard practice — roughly 40 KDoc blocks across 18 files, and `Log.w`/`Log.e` with `TAG` constants in four classes. A contributor following that document would have stripped the conventions the codebase actually relies on.
  - `STACK.md` had every version number wrong, and claimed `android.enableJetifier=true`, which is not set at all.
  - `CONCERNS.md` listed 20-odd items of which most were already fixed or deleted; the closed ones are now kept in a table with what closed them, so the history stays readable without being mistaken for open work.

  Each document now carries the commit it was verified against. Two findings surfaced during the pass and are recorded under "High-priority corrections" above rather than buried in the planning docs.
...
- `./gradlew lintDebug testDebugUnitTest assembleDebug` passes as of 2026-08-14: **48 tests across 5 classes, 0 failures, 0 skipped**; debug APK builds; lint reports **0 errors and 47 warnings**. Warnings fell from 50 to 47 when the dead resources were removed, which is the only change; no warning was suppressed.
- The test count moved 67 → 43 → 48 across the day: 67 was before the accessibility rewrite, 43 after it deleted `ForegroundAppDetectorTest` and `QuoteRepositoryTest` with the code they covered, and 48 once `HomeViewModelTest` was added. It is still 19 short of where it was, and the missing coverage is detection — see blocker 6.
